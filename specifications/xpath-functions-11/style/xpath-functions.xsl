...
<xsl:template match="body">
  <div class="toc">
        ...
        <p>
          <ul>
          <xsl:variable name="names" select="distinct-values(//proto[not(@role='example')]/@name)"/>
          <xsl:for-each-group select="$names" group-by="lower-case(substring(.,1,1))">
            <xsl:sort select="current-grouping-key()"/>
            <li><p>
            <xsl:for-each select="current-group()">
              <xsl:sort select="." lang="en"/>
              <a href="#func-{.}"><xsl:value-of select="."/></a>
              <xsl:text>&#xa0; </xsl:text>
            </xsl:for-each>
            </p></li>
          </xsl:for-each-group>
...
<xsl:template match="code">
  ...
  <xsl:variable name="name">
    <xsl:value-of select="normalize-space($raw-name)"/>
  </xsl:variable>
