...
<xsl:variable name="lang" select="document('../../xquery-11/src/xquery.xml',.)"/>
<xsl:variable name="ser" select="document('../../xslt-xquery-serialization-11/xslt-xquery-serialization.xml',.)"/>
...
<xsl:template match="processing-instruction('error-summary')">
  ...
  <dl>
    ...
    <xsl:apply-templates select="$xslt//error" mode="error-list">
      ...
      <xsl:attribute name="xlink:href">
        <xsl:choose>
          <xsl:when test="$spec = 'FO'">
            <xsl:text>http://www.w3.org/TR/xpath-functions-11/#ERR</xsl:text>
          </xsl:when>
          <xsl:when test="$spec = 'XT'">
            <xsl:text>http://www.w3.org/TR/xslt-21/#err-</xsl:text>
          </xsl:when>
          <xsl:when test="$spec = 'SE'">
            <xsl:text>http://www.w3.org/TR/xslt-xquery-serialization-11/#ERR</xsl:text>
          </xsl:when>
          <xsl:when test="$spec = 'XQ'">
            <xsl:text>http://www.w3.org/TR/xquery-11/#ERR</xsl:text>
          </xsl:when>
          <xsl:otherwise>
            <xsl:text>http://www.w3.org/TR/xpath-21/#ERR</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
        <xsl:value-of select="$spec"/>
        <xsl:value-of select="$class"/>
