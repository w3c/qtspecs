...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
    ...
    <xsl:template name="substitutions">
        <xsl:text>/* macros */&#10;</xsl:text>
        <xsl:call-template name="declare-unique-definitions">
            <xsl:with-param name="refs" 
                select="*[self::g:token 
                or self::g:skip][not(@special='yes')]//g:ref/@name"/>
                <!-- Note: Removed [not(@special='yes')] from above. -->
        </xsl:call-template>
        <xsl:text>&#10;</xsl:text>
        <xsl:apply-templates select="g:skip"/>
    </xsl:template>
    
    <!-- Declare the unique definitions (Substitutions). -->
    <xsl:template name="declare-unique-definitions">
        <xsl:param name="refs"/>
        <!-- required, list of all state attrs. -->
        <xsl:param name="attr-index" select="1"/>
        <xsl:param name="refs-str" select="''"/>
        <xsl:param name="last" select="count($refs)"/>
        <xsl:choose>
            <xsl:when test="$attr-index &lt;= $last">
                <xsl:call-template name="declare-unique-definitions">
                    <xsl:with-param name="refs-str" 
                        select="concat($refs-str, 
                    normalize-space($refs[$attr-index]), ' ')"/>
                    <xsl:with-param name="refs" select="$refs"/>
                    <xsl:with-param name="attr-index" select="$attr-index+1"/>
                    <xsl:with-param name="last" select="$last"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="tokenize-and-output-definition">
                    <xsl:with-param name="string" select="$refs-str"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    
    <!-- This is almost the tokenize-and-output template in utils.xsl, but 
       it needs to chase down the ref, unfortunately. -->
    <xsl:template name="tokenize-and-output-definition">
        <xsl:param name="string" select="''"/>
        <xsl:param name="used-states" select="' '"/>
        <xsl:if test="$string">
            <xsl:variable name="token" select="substring-before($string,' ')"/>
            <xsl:if test="$token">
                <xsl:if 
                    test="not(contains($used-states, concat(' ', $token, ' ')))">
                    <xsl:value-of select="$token"/>
                    <xsl:text> = </xsl:text>
                    <!-- added for jflex. -sb -->
                    <xsl:for-each select="key('ref',$token)/self::g:token">
                        <xsl:choose>
                            <xsl:when test="@alias-for">
                                <xsl:for-each 
                                    select="key('ref', @alias-for)/self::g:token">
                                    <xsl:call-template 
  ...
  -->
    ...
    <xsl:template name="rules">
        ...
        <!--<xsl:text>&#10;%%&#10;</xsl:text>-->
        <xsl:for-each select="g:skip">
            <xsl:text>&lt;</xsl:text>
            <xsl:call-template name="tokenize-and-output">
                <xsl:with-param name="string" select="concat(normalize-space(@recognize), ' ')"/>
...
import java.util.Stack;
  ...
  <xsl:template name="tokenize-and-output-actions">
    ...
    <xsl:template match="g:token" name="do-token">
        ...
        <xsl:variable name="rule">
            ...
            <xsl:choose>
            	<xsl:when test="@alias-for">
                    <xsl:for-each select="key('ref', @alias-for)/self::g:token">
                        <xsl:call-template name="apply-delimited-children">
                            <xsl:with-param name="delimeter" select="' '"/>
                        </xsl:call-template>            	    
                    </xsl:for-each>            	    
            	...
            	<xsl:otherwise>
                    ...
                    <xsl:text>:::</xsl:text> -->
                    <xsl:if test="not(//g:skip//g:ref[@name=$token-name])">
                        <xsl:call-template name="apply-delimited-children">
                            <xsl:with-param name="delimeter" select="' '"/>
                        </xsl:call-template>     
                    </xsl:if>  
            ...
            <xsl:if test="not(//g:skip//g:ref[@name=$token-name])">
                ...
                <xsl:choose>
                    <xsl:when test="key('ref', $token-name)/@skip='yes' or key('ref', $token-name)/@type='skip-token' ">
                        <xsl:text> /* ignore */ </xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:choose>
        ...
        <xsl:if test="not($lexStatesUnderWhichTokenIsRecognized/@name = 'ANY')
                  and not($lexStatesUnderWhichTokenIsRecognized/@name = 'DUMMY')">
            <xsl:if test="not(//g:skip//g:ref[@name=$token-name])">
