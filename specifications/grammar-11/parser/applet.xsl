...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="e-lang-with-links">
    <a href="{$spec_info/@uri-ref}">
      <xsl:value-of select="$spec_info/@full-title"/>
    </a>
    <xsl:if test="$spec2 != 'dummy'">
      <xsl:text> with </xsl:text>
      <a href="{$spec2_info/@uri-ref}">
        <xsl:value-of select="$spec2_info/@full-title"/>
      </a>
    </xsl:if>
    <xsl:if test="$spec3 != 'dummy'">
      <xsl:text> and </xsl:text>
      <a href="{$spec3_info/@uri-ref}">
        <xsl:value-of select="$spec3_info/@full-title"/>
      </a>
    </xsl:if>
  </xsl:template>
