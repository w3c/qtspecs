...
import org.w3c.dom.NodeList;
  ...
  <xsl:template name="action-production">
    ...
    <xsl:param name="prod-name" select="dummy"/>
      <!-- The name of the owning production.
      -->
    ...
    <xsl:param name="qualify-root-creation"
      ...
      <!-- True if the ASTNode subroot should
           be created only if it doesn't already exist.
      -->
    <xsl:param name="refset"
      select="$prod/*"/>
      <!-- The actual refset to which the ASTNodes should be built on.
      -->
    ...
    <xsl:choose>
      ...
      <xsl:when test="$prod/self::g:primary
        ...
        <xsl:variable name="exactly_one_typed_token"
            select="count($refset[key('map_name_to_defn',@name)/self::g:token/@value-type]) = 1"/>
          <!--
            True iff $refset contains exactly one element whose 'name' attribute
            maps to a g:token element with a 'value-type' attribute.
          -->

        <xsl:for-each select="$refset">
          <xsl:call-template name="action-for-child">
            ...
            <xsl:with-param name="exactly_one_typed_token"
              select="$exactly_one_typed_token"/>
          </xsl:call-template>
        </xsl:for-each>


        <xsl:if test="$prod/self::g:prefix">
          <!-- For the implied ref after the prefix -->
          <xsl:variable name="count" select="count($prod/*)"/>
          <xsl:value-of select="$break-and-indent"/>
  ...
  <xsl:template name="action-for-child">
    <xsl:param name="break" select="' '"/>
    <xsl:param name="pseudo-variable-number-offset" select="0"/>
    <xsl:param name="exactly_one_typed_token"/>

    <xsl:variable name="N" select="position() + $pseudo-variable-number-offset"/>
    <xsl:variable name="dollarN_var_name" select="concat('$', $N)"/>
    <xsl:variable name="astN_var_name" select="concat('astn', $N)"/>

    <xsl:choose>
      <xsl:when test="./self::g:string">
        <!-- skip: doesn't add anything to AST -->
      </xsl:when>

      <xsl:when test="./self::g:ref">
        <xsl:variable name="token" select="key('map_name_to_defn', ./@name)/self::g:token"/>
        <xsl:choose>
          <xsl:when test="$token/@value-type">

            <xsl:variable name="token_value_type" select="normalize-space($token/@value-type)"/>
            <xsl:variable name="ASTNodeSetValue_func_name">
              <xsl:choose>
                <xsl:when test="$token_value_type='string'">
                  <xsl:text>ASTNodeSetValueToString</xsl:text>
                </xsl:when>
                <xsl:when test="$token_value_type='number'">
                  <xsl:text>ASTNodeSetValueToNumber</xsl:text>
                </xsl:when>
                <xsl:when test="$token_value_type='id'">
            ...
            <xsl:choose>
              <xsl:when test="$exactly_one_typed_token">
                <!--
                  $refset contains exactly one element whose 'name' attribute
                  maps to a g:token element with a 'value-type' attribute,
                  and the current node is it.
                -->
                <xsl:value-of select="$break"/>
                <xsl:value-of select="$ASTNodeSetValue_func_name"/>
                <xsl:text>($$, </xsl:text>
                <xsl:value-of select="$dollarN_var_name"/>
                <xsl:text>);</xsl:text>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="$break"/>
                <xsl:text>{ SimpleNode </xsl:text>
                <xsl:value-of select="$astN_var_name"/>
