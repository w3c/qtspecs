...
<!-- ????                       xqx:catchErrorCode | xqx:catchErrorDesc | xqx:catchErrorVal | -->
...
<!-- Surrounding FLWOR expressions with parentheses completes the set -->
  ...
  <xsl:template match="xqx:catchClause">
    ...
    <xsl:apply-templates select="xqx:catchVars"/>
  ...
  <xsl:template match="xqx:catchVars">
    <xsl:value-of select="$NEWLINE"/>
    <xsl:value-of select="$LPAREN"/>
    <xsl:value-of select="$SPACE"/>
    <xsl:apply-templates select="xqx:catchErrorCode"/>
    <xsl:apply-templates select="xqx:catchErrorDesc"/>
    <xsl:apply-templates select="xqx:catchErrorVal"/>
    <xsl:value-of select="$SPACE"/>
    <xsl:value-of select="$RPAREN"/>
  </xsl:template>


  <xsl:template match="xqx:catchErrorCode">
    <xsl:apply-templates/>
  </xsl:template>


  <xsl:template match="xqx:catchErrorDesc">
    <xsl:value-of select="$COMMA_SPACE"/>
    <xsl:apply-templates/>
  </xsl:template>


  <xsl:template match="xqx:catchErrorVal">
    <xsl:value-of select="$COMMA_SPACE"/>
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="xqx:validateExpr">
    ...
    <xsl:if test="xqx:sequenceType">
      <xsl:text>as </xsl:text>
      <xsl:value-of select="xqx:sequenceType"/>
      <xsl:value-of select="$SPACE"/>
    </xsl:if>
    <xsl:value-of select="$LBRACE"/>
    <xsl:apply-templates select="xqx:argExpr"/>
  ...
  <xsl:template match="xqx:decimalFormatDecl">
    <xsl:if test="@xqx:default = 'true' and xqx:decimalFormatName">
      <xsl:message terminate="yes">Incorrect XQueryX:
          xqx:decimalFormatDecl does not allow both @default=true and xqx:decimalFormatName</xsl:message>
    </xsl:if>
